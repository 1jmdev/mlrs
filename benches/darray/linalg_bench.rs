...
fn bench_linalg(c: &mut Criterion) {
    ...
    g.bench_function("dot_1d", |b| b.iter(|| v1.dot(black_box(&v2))));
    g.bench_function("matmul_512", |b| b.iter(|| m1.matmul(black_box(&m2))));
    g.bench_function("outer_2k", |b| {
        ...
        b.iter(|| a.outer(black_box(&b2)))
    });
    g.bench_function("diag_extract", |b| b.iter(|| m1.diag(black_box(0))));
    g.finish();
}
