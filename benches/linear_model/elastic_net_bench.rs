...
use mlrs::linear_model::ElasticNet;

fn data(samples: usize, features: usize) -> (Array, Array) {
    let x = Array::from_shape_vec(
        &[samples, features],
        (0..samples * features)
            .map(|i| ((i % 29) as f64) * 0.03)
            .collect(),
    );
    let y = Array::from_shape_vec(
        &[samples],
        (0..samples).map(|i| ((i % 19) as f64) * 0.7).collect(),
    );
    (x, y)
}
...
fn bench_elastic_net(c: &mut Criterion) {
    ...
    let mut trained = ElasticNet::new(0.05).l1_ratio(0.4).max_iter(200).tol(1e-4);
    trained.fit(&x, &y).expect("elastic net fit failed");
    let mut g = c.benchmark_group("linear_model_elastic_net");
    g.bench_function("fit", |b| {
        b.iter(|| {
            let mut model = ElasticNet::new(black_box(0.05))
                .l1_ratio(black_box(0.4))
...
criterion_group!(benches, bench_elastic_net);
